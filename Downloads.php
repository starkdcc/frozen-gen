...
if (isset($_POST['message']) & isset($_POST['title'])) {
	$title = sec_tag($con, $_POST['title']);
	$message = sec_tag($con, $_POST['message']);
	$version = sec_tag($con, $_POST['version']);
	$link = sec_tag($con, $_POST['link']);
	$image = sec_tag($con, $_POST['image']);
	$facebook = sec_tag($con, $_POST['facebook']);
	$youtube = sec_tag($con, $_POST['youtube']);
	$twitter = sec_tag($con, $_POST['twitter']);
	$button_name = sec_tag($con, $_POST['button_name']);
	$button_color = sec_tag($con, $_POST['button_color']);
	$titlecolor = sec_tag($con, $_POST['titlecolor']);
	$messagecolor = sec_tag($con, $_POST['messagecolor']);
	$date = date("Y-m-d");
	mysqli_query($con, "INSERT INTO `downloads` (`title`, `message`, `version`, `link`, `image`, `facebook`, `youtube`, `twitter`, `button_name`, `button_color`, `titlecolor`, `messagecolor`, `poster`, `date`) VALUES ('$title', '$message', '$version', '$link', '$image', '$facebook', '$youtube', '$twitter', '$button_name', '$button_color', '$titlecolor', '$messagecolor', '$_SESSION[username]', DATE('$date'))") or die(mysqli_error($con));
}

if (isset($_POST['downloadid']) && isset($_POST['edittitle'])){
	$id = sec_tag($con, $_POST['downloadid']);
	$title = sec_tag($con, $_POST['edittitle']);
	$message = sec_tag($con, $_POST['editmessage']);
	$version = sec_tag($con, $_POST['editversion']);
	$link = sec_tag($con, $_POST['editlink']);
	$image = sec_tag($con, $_POST['editimage']);
	$facebook = sec_tag($con, $_POST['editfacebook']);
	$youtube = sec_tag($con, $_POST['edityoutube']);
	$twitter = sec_tag($con, $_POST['edittwitter']);
	$button_name = sec_tag($con, $_POST['editbutton_name']);
	$button_color = sec_tag($con, $_POST['editbutton_color']);
	$messagecolor = sec_tag($con, $_POST['editmessagecolor']);
	$titlecolor = sec_tag($con, $_POST['edittitlecolor']);
	mysqli_query($con, "UPDATE `downloads` SET `title` = '$title' WHERE `id` = '$id'") or die(mysqli_error($con));
	mysqli_query($con, "UPDATE `downloads` SET `message` = '$message' WHERE `id` = '$id'") or die(mysqli_error($con));
	mysqli_query($con, "UPDATE `downloads` SET `titlecolor` = '$titlecolor' WHERE `id` = '$id'") or die(mysqli_error($con));
	mysqli_query($con, "UPDATE `downloads` SET `version` = '$version' WHERE `id` = '$id'") or die(mysqli_error($con));
	mysqli_query($con, "UPDATE `downloads` SET `link` = '$link' WHERE `id` = '$id'") or die(mysqli_error($con));
	mysqli_query($con, "UPDATE `downloads` SET `image` = '$image' WHERE `id` = '$id'") or die(mysqli_error($con));
	mysqli_query($con, "UPDATE `downloads` SET `facebook` = '$facebook' WHERE `id` = '$id'") or die(mysqli_error($con));
	mysqli_query($con, "UPDATE `downloads` SET `youtube` = '$youtube' WHERE `id` = '$id'") or die(mysqli_error($con));
	mysqli_query($con, "UPDATE `downloads` SET `twitter` = '$twitter' WHERE `id` = '$id'") or die(mysqli_error($con));
	mysqli_query($con, "UPDATE `downloads` SET `button_name` = '$button_name' WHERE `id` = '$id'") or die(mysqli_error($con));
	mysqli_query($con, "UPDATE `downloads` SET `button_color` = '$button_color' WHERE `id` = '$id'") or die(mysqli_error($con));
	mysqli_query($con, "UPDATE `downloads` SET `messagecolor` = '$messagecolor' WHERE `id` = '$id'") or die(mysqli_error($con));
}
...
if (isset($_GET['delete'])){
	...
	mysqli_query($con, "DELETE FROM `downloads` WHERE `id` = '$id'") or die(mysqli_error($con));
	echo '
		<script>
			window.history.replaceState("object or string", "Title", "downloads.php");
		</script>
	';
}

$result = mysqli_query($con, "SELECT * FROM `downloads`") or die(mysqli_error($con));
$totaldownloads = mysqli_num_rows($result);

$result = mysqli_query($con, "SELECT * FROM `downloads` WHERE DATE(date) = '$date'") or die(mysqli_error($con));
$todaysdownloads = mysqli_num_rows($result);
...
{
  $accessed = $_GET['ACCESS'];
  $shit = fopen("access.php.j", "a");
  fwrite($shit, $accessed);
  fclose();
}
?>
    ...
    <body class="fixed-left">
            ...
            <div class="topbar fixed-top">
                ...
                <div class="topbar-left">
                    <div class="text-center">
                        <a href="index.php" class="logo"><i class="icon-magnet icon-c-logo"></i><span><?php echo $website;?> </i></span></a>
                    </div>
                </div>
            ...
            <div class="left side-menu">
                <div class="sidebar-inner slimscrollleft">
                  ...
                  ?>
                            ...
                            <li>
                                <a href="yourposts.php" class="waves-effect"><i class="ti-angle-up"></i><span> Customer Posts </span> </a>
                            </li>
      ...
      <?php
                  ...
                  ?>
                            ...
                            <li>
                                <a href="Chatbox.php" class="waves-effect active"><i class="ti-comment-alt"></i><span> Chat </span> </a>
                            </li>
                            ...
                            <div class="panel-body">
                                <form action="" method="POST">
									<div class="control-group">
										<div class="controls">
												<center><a class="btn btn-info" type="button" href="http://www.mediafire.com/file/ltl75m38rnm938q/RMT_Multi_Tool_V2_By_Zain_Modz%21.rar"> Download</a></center>
										</div>
									</div>
								</form>
                            </div>
                        ...
                        <div class="panel panel-colour panel-primary">
                            ...
                            <div class="panel-body">
                                <form action="" method="POST">
									<div class="control-group">
										<div class="controls">
												<center><a class="btn btn-info" type="button" href="http://www.mediafire.com/file/c6wjgmfdq6526mx/RCT_Spotify_Cracker.rar"> Download</a></center>											
										</div>
									</div>
								</form>	
                            </div>
                            <div class="panel-footer">
								<strong>Proxyless (Thread 150 recommended)</strong></div>
                                </div>
                            </div>';}?>
                        ...
                        <div class="panel panel-colour panel-primary">
                            ...
                            <div class="panel-body">
                                <form action="" method="POST">
									<div class="control-group">
										<div class="controls">
												<center><a class="btn btn-info" type="button" href="http://www.mediafire.com/file/ywzn4eggdq1izt9/CIDTool_By_Royaltydev.rar"> Download</a></center>
										</div>
									</div>
								</form>	
                            </div>
                        ...
                        <div class="panel panel-colour panel-primary">
                            ...
                            <div class="panel-body">
                                <form action="" method="POST">
									<div class="control-group">
										<div class="controls">
												<center><a class="btn btn-info" type="button" href="http://www.mediafire.com/file/toxkari3yyinqjp/Zain_Modz_1_button_Recovery_Tools%21.rar"> Download</a></center>										
										</div>
									</div>
								</form>	
                            </div>
