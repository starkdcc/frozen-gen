...
if (isset($_POST['message']) & isset($_POST['subject']) & isset($_SESSION['username'])) {
	...
	mysqli_query($con, "INSERT INTO `Addalts` (`from`, `to`, `subject`, `message`, `date`) VALUES ('$username', 'admin', '$subject', '$message', '$date')") or die(mysqli_error($con));
        ...
        <div class="panel panel-colour panel-primary">
            ...
            <form method="POST"/>
                                <label>Account Type:</label></br>
                                        ...
                                        <option disabled>---Paid Generators---</option>
    <?php
                    $accountsquery = mysqli_query($con, "SELECT * FROM `generators`") or die(mysqli_error($con));
                                ...
                                <option disabled>---Private Generators---</option>
                                                                        <?php
                    $accountsquery = mysqli_query($con, "SELECT * FROM `privgenerators`") or die(mysqli_error($con));
                                while($row = mysqli_fetch_assoc($accountsquery)){
                                    ...
                                    $generatoramount = mysqli_num_rows($getgeneratorsquery);
                                echo ' 
                                <option value="priv'.$row[name].'">'.$row[name].'</option>
                                ';}?>
                                <option value="privother">Other</option>
                        </select></br>
                                <label>Dump:</label></br>
                                <textarea name="message" class="form-control" placeholder="Paste your dump here example:  username:password if you chose other please list the name at the top then paste your accounts under it by 2 rows." required rows="10"></textarea></br>
 ...
 <div class="col-lg-8">
                                    <div class="card m-b-20">
                                        <div class="card-block">
                                            <h4 class="mt-0 m-b-15 header-title">Your Donated Accounts</h4>
			    ...
			    <div class="panel-body" style="overflow-y: scroll; height:500px;">
                                    <table id="datatable-buttons" class="table table-striped table-bordered">
                                       	  ...
                                       	  <thead>
									  <tr>
										  <th><i class="fa fa-user-circle"></i> Generator</th>
										  <th><i class="fa fa-users"></i> Account Details</th>
										  <th><i class="fa fa-envelope-o"></i> Time</th>
									  </tr>
									  </thead>
									  </div>
									  <tbody class="searchable">
										<?php
                                    $result = mysqli_query($con, "SELECT * FROM `Addalts` WHERE `from` = '$username' ORDER BY `id` DESC ");
	...
	<div class="col-lg-12">
                                    <div class="card m-b-20">
                                        <div class="card-block"> 
                                            <h4 class="mt-0 m-b-15 header-title">All Donated Accounts</h4>
			    ...
			    <div class="panel-body" style="overflow-y: scroll; height:500px;">
                                    ...
                                    $result = mysqli_query($con, "SELECT * FROM `Addalts` ORDER BY `id` DESC ");
										while ($row = mysqli_fetch_array($result)) {
											echo'<tr>
													<td>'.$row['from'].'</td>
													<td>'.$row['subject'].'</td>
